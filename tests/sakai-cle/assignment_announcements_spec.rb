...
describe "Assignments appearing in Announcements" do
  ...
  before :all do
    ...
    @assignment = make AssignmentObject, :status=>"Draft", :add_open_announcement=>:set, :site=>@site.name, :title=>random_xss_string(30), :open=>in_an_hour, :student_submissions=>"Attachments only", :grade_scale=>"Points", :max_points=>"100", :instructions=>random_multiline(600, 12, :string)
  ...
  it "Assignments in Draft status do not appear in Announcements" do
    ...
    on Home do |home|
     home.announcements_list.should_not include("Assignment: Open Date for '#{@assignment.title}'")
    end
